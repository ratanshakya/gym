...
try {
  transporter = nodemailer.createTransport({
    ...
    port: process.env.VITE_SMTP_PORT || process.env.SMTP_PORT || 587,
    secure: false,
    auth: {
      user: process.env.VITE_SMTP_USER || process.env.SMTP_USER,
      pass: process.env.VITE_SMTP_PASS || process.env.SMTP_PASS,
    },
  });
...
app.post('/api/send-email', async (req, res) => {
  try {
    ...
    const info = await transporter.sendMail({
      from: `"${fromName || 'EasyGym Support'}" <${process.env.VITE_SMTP_USER || process.env.SMTP_USER}>`,
      replyTo: replyTo || process.env.VITE_SMTP_USER || process.env.SMTP_USER,
      to,
      subject,
      text,
      html,
...
app.put('/api/products/:id', async (req, res) => {
  try {
    const searchId = req.params.id;
    if (mongoose.connection.readyState === 1) {
      const updated = await Product.findOneAndUpdate({ id: searchId }, req.body, { new: true });
      return res.json({ success: true, message: "Product updated in MongoDB", data: updated });
    }
    res.json({ success: true, message: "Product updated locally", data: req.body });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
});
...
// Serve the static files from the React app (built Vite project)
app.use(express.static(path.join(__dirname, 'dist')));

// Handles any requests that don't match the API above by sending back the React app
app.get('*', (req, res) => {
